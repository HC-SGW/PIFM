...
def mask_adjs(adjs, flags):
    ...
    if flags is None:
        ...
        flags = flags.to(adjs.device)

    if adjs.dim() == 4:
        flags = flags.unsqueeze(1)  # B x 1 x N

    adjs = adjs * flags.unsqueeze(-1)  # mask columns
    adjs = adjs * flags.unsqueeze(-2)  # mask rows
...
def pow_tensor(x, cnum):
    ...
    for i in range(cnum - 1):
        ...
        # 🚨 Check for NaNs right after multiplication
        if torch.isnan(x_).any():
            print(f"[❌] NaN detected at power {i+2} in pow_tensor")
            print(f"    x_.min() = {x_.min().item()}, x_.max() = {x_.max().item()}")
            print(f"    x_.mean() = {x_.mean().item()}")
            print(f"    Any inf? {torch.isinf(x_).any()}")
            break
    ...
    if torch.isnan(xc).any():
        print(f"[❌] Final xc contains NaNs")
    '''else:
        print(f"[✅] pow_tensor completed. xc shape = {xc.shape}, max = {xc.max().item()}, min = {xc.min().item()}")'''
