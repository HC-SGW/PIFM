#!/usr/bin/env bash
# GraphSAGE-HEART prior run for Cora, edge-centered k=3 (feature adapter ON)

set -euo pipefail

export CUDA_VISIBLE_DEVICES=2

RUN_NAME="CORA_pifm_GraphSAGEHeartPrior_edgeCentered_k4"
EPOCHS=10             # PIFM epochs
HEART_EPOCHS=10      # GraphSAGE-HEART epochs (full-graph pretrain)
LOG_DIR="logs"
...
nohup bash -c "
  echo '=== START (GraphSAGE-HEART prior, edge-centered k=4) ' \$(date) ' ==='
  echo 'Run: ${RUN_NAME}'
  echo 'Log: ${LOG_FILE}'
  ...
  python -u main.py --run_name ${RUN_NAME} train_expansion \
    ...
    --k_hop 4 --max_nodes 256 --target_coverage 1 \
  ...
  echo
  echo '=== END (GraphSAGE-HEART prior, edge-centered k=3) ' \$(date) ' ==='
" > "$LOG_FILE" 2>&1 &

echo "Launched edge-centered k=3 run. PID: $!"
echo "Tail logs with:"
